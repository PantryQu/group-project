...
const UserChat = () => {
    ...
    const q = query(collection(db, "chats"), where("uid", "==", localStorage.getItem("uid")))
    ...
    if (loading) {
        return (
            <div className="w-full h-screen flex justify-center items-center">
                <div className="w-16 h-16 border-dashed rounded-full animate-spin border-slate-800"></div>
            </div>
        )
    }
    ...
    const createMessage = async () => {
        const res = await addDoc(collection(db, `chats`), {
            ...
            name: currentUser[0].displayName ? currentUser[0].displayName : currentUser[0].email, timestamp: serverTimestamp()
        })
